...
def parseData(scenario):
    ...
    for line in file.readlines():
        if not (line.find("complete generation") == -1):
            ...
            data["no contact"] = data["no contact"] + float(
                line.rstrip("\n").split()[2]
            )
        elif not (line.find("unstable contact:") == -1):
            data["unstable contact"] = data["unstable contact"] + float(
                line.rstrip("\n").split()[2]
            )
...
def printOneStat(f, name, data, g_time, tTime):
    ...
    f.write(n + " tests: \n")
    f.write(
        "\t single operation time (min / avg / max):\n \t "
        + str(d[n + "_min"])
        + "\t"
        + str(d[n])
        + "\t"
    ...
    f.write(
        "\t total time (min / avg / max / % of total / % of total wtht path planning):\n \t "
        + str(d["total_" + n + "_min"])
        + "\t"
        + str(d["total_" + n])
        + "\t"
        + str(d["total_" + n + "_max"])
        + "\t"
        + str(float(d["total_" + n]) / tTime * 100)
        + "%"
        + "\t"
        + str(float(d["total_" + n]) / g_time * 100)
        + "%\n"
    )
    f.write(
...
def analyzeData():
    ...
    for scenario in scenarios:
        ...
        f.write(
            "total computation time (min / avg / max): "
            + str(d["mingen_time"] + d["min_path_time"])
            + "\t"
            + str(tTime)
            + "\t"
            + str(d["maxgen_time"] + d["max_path_time"])
            + "\n"
        )

        f.write(
            "generation time (without RRT) (min / avg / max / % of total):\n \t "
            + str(d["mingen_time"])
            + "\t"
            + str(d["gen_time"])
            + "\t"
            + str(d["maxgen_time"])
            + "\t"
            + str(float(d["gen_time"]) / tTime * 100)
            + "%\n"
        )

        f.write("path computation: \n")
        f.write(
            "\t time (min / avg / max / % of total):\n \t "
            + str(d["min_path_time"])
            + "\t"
            + str(d["path_time"])
            + "\t"
            + str(d["max_path_time"])
            + "\t"
            + str(float(d["path_time"]) / tTime * 100)
            + "%\n"
        )
        ...
        f.write(
            "number of successes / failures / ratio: "
            + str(d["success"])
            + "\t"
            + str(d["failed"])
            + "\t"
            + str(float(d["success"]) / float(d["success"]) * 100)
            + "%\n"
        )
        ...
        tc = nc + c + uc
        f.write(
            "% of failed contact generation (no candidates found): "
            + str(nc / tc * 100)
            + "%\n"
        )
        f.write(
            "% of unstable contact generation (no balanced candidates found): "
            + str(uc / tc * 100)
            + "%\n"
        )
        f.write("\n")
        ...
        if d["optim_num_trials"] > 0:
            ...
            f.write(
                "% succes rate of optimization: "
                + str(d["optim_num_success"] / d["optim_num_trials"] * 100)
                + "%\n"
            )
            f.write(
                "% succes rate of optimization disregarding collision: "
                + str(
                    d["optim_num_success"]
                    / (d["optim_num_trials"] - d["optim_error_com_proj"])
                    * 100
                )
                + "%\n"
            )
            if d["num_errors"] > 0:
                f.write(
                    "% errors due to problem infeasibility (% of errors, % over all trials): "
                    + str(d["optim_error_optim_fail"] / d["num_errors"] * 100)
                    + " "
                    + str(d["optim_error_optim_fail"] / d["optim_num_trials"] * 100)
                    + "%\n"
                )
                f.write(
                    "% errors due to unknown reasons (% of errors, % over all trials): "
                    + str(d["optim_error_unknown"] / d["num_errors"] * 100)
                    + " "
                    + str(d["optim_error_unknown"] / d["optim_num_trials"] * 100)
                    + "%\n"
                )
