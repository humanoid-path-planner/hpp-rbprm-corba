...
struct BindShooter {
  ...
  hpp::rbprm::RbPrmShooterPtr_t create(
      /*const hpp::pinocchio::DevicePtr_t& robot,*/ const hpp::core::
          ProblemConstPtr_t& problem) {
    ...
    shooter->sampleExtraDOF(
        problem->getParameter("ConfigurationShooter/sampleExtraDOF")
            .boolValue());
    shooter->ratioWeighted(
        problem->getParameter("RbprmShooter/ratioWeighted").floatValue());
    return shooter;
  }
  ...
  hpp::core::PathValidationPtr_t createPathValidation(
    ...
    problemSolver_->problem()->configValidation(
  ...
  hpp::core::PathValidationPtr_t createDynamicPathValidation(
    ...
    sizeFootX = problemSolver_->problem()
                    ->getParameter(std::string("DynamicPlanner/sizeFootX"))
                    .floatValue() /
                2.;
    sizeFootY = problemSolver_->problem()
...
class FullBodyMap {
 public:
  ...
  ReturnType keys() const {
    ReturnType l;
    for (fMap_t::const_iterator it = map_.begin(); it != map_.end(); ++it)
...
class RbprmBuilder : public virtual POA_hpp::corbaserver::rbprm::RbprmBuilder {
 public:
  ...
  void usePosturalTaskContactCreation(
      const bool usePosturalTaskContactCreation);

  virtual void setFilter(const hpp::Names_t& roms);
  virtual void setAffordanceFilter(const char* romName,
                                   const hpp::Names_t& affordances);
  virtual void boundSO3(const hpp::floatSeq& limitszyx);

  virtual hpp::floatSeq* getSampleConfig(const char* limb,
                                         unsigned int sampleId);
  virtual hpp::floatSeq* getSamplePosition(const char* limb,
                                           unsigned int sampleId);
  virtual hpp::floatSeqSeq* getEffectorPosition(
      const char* limb, const hpp::floatSeq& configuration);
  virtual CORBA::UShort getNumSamples(const char* limb);
  virtual hpp::floatSeq* getOctreeNodeIds(const char* limb);
  virtual double getSampleValue(const char* limb, const char* valueName,
                                unsigned int sampleId);
  virtual double getEffectorDistance(unsigned short state1,
  ...
  virtual hpp::floatSeq* generateContacts(const hpp::floatSeq& configuration,
                                          ...
                                          const hpp::floatSeq& acceleration,
                                          const double robustnessThreshold);
  virtual CORBA::Short generateStateInContact(
  ...
  virtual hpp::floatSeq* generateGroundContact(
      const hpp::Names_t& contactLimbs);

  virtual hpp::floatSeq* getContactSamplesIds(
      const char* limb, const hpp::floatSeq& configuration,
      const hpp::floatSeq& direction);

  virtual hpp::floatSeqSeq* getContactSamplesProjected(
      const char* limb, const hpp::floatSeq& configuration,
      const hpp::floatSeq& direction, unsigned short numSamples);

  virtual short generateContactState(::CORBA::UShort currentState,
                                     const char* name,
                                     const ::hpp::floatSeq& direction);
  ...
  virtual void addLimb(const char* id, const char* limb, const char* effector,
                       const hpp::floatSeq& offset, const hpp::floatSeq& normal,
                       double x, double y, unsigned int samples,
                       const char* heuristicName, double resolution,
                       const char* contactType, double disableEffectorCollision,
                       double grasp, const hpp::floatSeq& limbOffset,
                       const char* kinematicConstraintsPath,
                       double kinematicConstraintsMin);
  virtual void addNonContactingLimb(const char* id, const char* limb,
                                    const char* effector, unsigned int samples);

  virtual void addLimbDatabase(const char* databasePath, const char* id,
                               const char* heuristicName, double loadValues,
                               double disableEffectorCollision, double grasp);

  virtual void setStartState(const hpp::floatSeq& configuration,
                             const hpp::Names_t& contactLimbs);
  virtual void setEndState(const hpp::floatSeq& configuration,
                           const hpp::Names_t& contactLimbs);
  virtual void setStartStateId(unsigned short stateId);
  virtual void setEndStateId(unsigned short stateId);
  virtual hpp::floatSeq* computeContactForConfig(
      const hpp::floatSeq& configuration, const char* limbNam);
  virtual hpp::floatSeqSeq* computeContactPoints(unsigned short cId);
  virtual hpp::floatSeqSeq* computeContactPointsAtState(
      unsigned short cId, unsigned short isIntermediate);
  virtual hpp::floatSeqSeq* computeContactPointsForLimb(unsigned short cId,
                                                        const char* limbName);
  virtual hpp::floatSeqSeq* computeContactPointsAtStateForLimb(
      unsigned short cId, unsigned short isIntermediate, const char* limbName);
  virtual hpp::floatSeqSeq* computeCenterOfContactAtStateForLimb(
      unsigned short cId, unsigned short isIntermediate, const char* limbName);
  virtual hpp::floatSeqSeq* interpolate(double timestep, double path,
                                        double robustnessTreshold,
                                        unsigned short filterStates,
                                        bool testReachability, bool quasiStatic,
                                        bool erasePreviousStates);
  virtual hpp::floatSeqSeq* interpolateConfigs(const hpp::floatSeqSeq& configs,
                                               double robustnessTreshold,
                                               unsigned short filterStates,
                                               bool testReachability,
                                               bool quasiStatic,
                                               bool erasePreviousStates);
  virtual hpp::floatSeqSeq* getContactCone(unsigned short stateId,
                                           double friction);
  virtual hpp::floatSeqSeq* getContactIntermediateCone(unsigned short stateId,
                                                       double friction);
  virtual CORBA::Short generateComTraj(const hpp::floatSeqSeq& positions,
                                       const hpp::floatSeqSeq& velocities,
  ...
  virtual CORBA::Short limbRRTFromRootPath(unsigned short state1,
                                           unsigned short state2,
                                           unsigned short path,
                                           unsigned short numOptimizations);
  virtual CORBA::Short configToPath(const hpp::floatSeqSeq& configs);
  virtual CORBA::Short comRRT(unsigned short state1, unsigned short state2,
                              unsigned short path,
                              unsigned short numOptimizations);

  typedef core::PathPtr_t (*t_rrt)(RbPrmFullBodyPtr_t, core::ProblemSolverPtr_t,
                                   const core::PathPtr_t, const State&,
                                   const State&, const std::size_t, const bool);

  hpp::floatSeq* rrt(t_rrt functor, unsigned short state1,
                     unsigned short state2, unsigned short comTraj1,
  ...
  virtual hpp::floatSeq* comRRTFromPos(unsigned short state1,
                                       ...
                                       unsigned short numOptimizations);
  virtual hpp::floatSeq* comRRTFromPosBetweenState(
      unsigned short state1, unsigned short state2, unsigned short comTraj1,
      unsigned short comTraj2, unsigned short comTraj3,
      unsigned short numOptimizations);
  virtual hpp::floatSeq* effectorRRTFromPosBetweenState(
      unsigned short state1, unsigned short state2, unsigned short comTraj1,
      unsigned short comTraj2, unsigned short comTraj3,
      unsigned short numOptimizations);
  virtual hpp::floatSeq* effectorRRT(unsigned short state1,
                                     ...
                                     unsigned short comTraj3,
                                     unsigned short numOptimizations);
  virtual hpp::floatSeq* effectorRRTFromPath(
      unsigned short state1, unsigned short path, double path_from,
  ...
  virtual hpp::floatSeq* rrtOnePhase(t_rrt functor, unsigned short state1,
                                     unsigned short state2,
                                     unsigned short comTraj,
                                     unsigned short numOptimizations);
  virtual hpp::floatSeq* effectorRRTOnePhase(unsigned short state1,
                                             unsigned short state2,
                                             unsigned short comTraj,
                                             unsigned short numOptimizations);
  virtual hpp::floatSeq* comRRTOnePhase(unsigned short state1,
                                        unsigned short state2,
                                        unsigned short comTraj,
                                        unsigned short numOptimizations);
  virtual hpp::floatSeqSeq* generateEffectorBezierArray(
      unsigned short state1, unsigned short state2, unsigned short comTraj,
      unsigned short numOptimizations);

  virtual CORBA::Short generateEndEffectorBezier(unsigned short state1,
                                                 unsigned short state2,
                                                 unsigned short cT);

  virtual hpp::floatSeq* projectToCom(unsigned short state,
  ...
  virtual double setConfigAtState(unsigned short stateId,
                                  const hpp::floatSeq& config);
  double projectStateToCOMEigen(State& s,
                                const pinocchio::Configuration_t& com_target,
                                unsigned short maxNumeSamples);
  double projectStateToCOMEigen(unsigned short stateId,
                                const pinocchio::Configuration_t& com_target,
                                unsigned short maxNumeSamples);

  virtual double projectStateToCOM(unsigned short stateId,
                                   const hpp::floatSeq& com,
                                   unsigned short max_num_sample);
  virtual CORBA::Short cloneState(unsigned short stateId);
  virtual double projectStateToRoot(unsigned short stateId,
  ...
  virtual CORBA::Short isLimbInContact(const char* limbName,
                                       unsigned short state);
  virtual CORBA::Short isLimbInContactIntermediary(const char* limbName,
                                                   unsigned short state);
  virtual CORBA::Short computeIntermediary(unsigned short state1,
                                           unsigned short state2);
  virtual CORBA::Short getNumStates();
  virtual hpp::floatSeqSeq* getOctreeBoxes(const char* limbName,
                                           const hpp::floatSeq& configuration);
  virtual hpp::floatSeq* getOctreeTransform(const char* limbName,
                                            const hpp::floatSeq& configuration);
  virtual CORBA::Short isConfigBalanced(const hpp::floatSeq& config,
                                        const hpp::Names_t& contactLimbs,
                                        double robustnessTreshold,
                                        const hpp::floatSeq& CoM);
  virtual double isStateBalanced(unsigned short stateId);
  virtual void runSampleAnalysis(const char* analysis, double isstatic);
  virtual hpp::floatSeq* runLimbSampleAnalysis(const char* limbname,
                                               const char* analysis,
                                               double isstatic);
  ...
  virtual Names_t* getContactsVariations(unsigned short stateIdFrom,
                                         unsigned short stateIdTo);
  virtual Names_t* getCollidingObstacleAtConfig(
      const ::hpp::floatSeq& configuration, const char* limbName);
  virtual floatSeqSeq* getContactSurfacesAtConfig(
      const ::hpp::floatSeq& configuration, const char* limbName);

  virtual Names_t* getAllLimbsNames();
  virtual CORBA::Short addNewContact(unsigned short stateId,
                                     const char* limbName,
                                     const hpp::floatSeq& position,
                                     const hpp::floatSeq& normal,
                                     unsigned short max_num_sample,
                                     bool lockOtherJoints,
                                     const floatSeq& rotation);
  virtual CORBA::Short removeContact(unsigned short stateId,
                                     const char* limbName);
  virtual hpp::floatSeq* computeTargetTransform(
  ...
  virtual hpp::floatSeqSeqSeq* getEffectorTrajectoryWaypoints(
  ...
  virtual bool areKinematicsConstraintsVerifiedForState(
      unsigned short stateId, const hpp::floatSeq& point);
  virtual hpp::floatSeq* isReachableFromState(unsigned short stateFrom,
                                              unsigned short stateTo,
                                              const bool useIntermediateState);
  virtual hpp::floatSeq* isDynamicallyReachableFromState(
