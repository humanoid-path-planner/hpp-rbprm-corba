...
class AbstractContactGenerator:
    ...
    weight_postural = (
        None  # weight used for the postural task (depending on the setting)
    )
    ...
    def __init__(self, path_planner):
        ...
        # increase bounds from path planning, to leave room for the root motion during
        # the steps.
        for i in range(3):
        ...
        # (see https://github.com/humanoid-path-planner/hpp-centroidal-dynamics/blob/ \
        # master/include/hpp/centroidal-dynamics/centroidal_dynamics.hh#L215)
        ...
        # if False, keep^the previously computed states if 'interpolate' is called a
        # second time.
        self.erase_previous_states = True
        # if True, the acceleration computed by the guide is ignored during contact
        # planning.
        self.static_stability = True
        # limbs in contact in the initial configuration
        self.init_contacts = self.used_limbs
        # the order of the limbs in the list define the initial order in which the
        # contacts are removed when then cannot be maintained anymore.
    ...
    def set_reference(self, use_postural_task=True):
        ...
        :param use_postural_task: if True, when projecting configurations to contact a
        postural task is added to the cost function.
        Disabling this setting will greatly reduce the computation time, but may result
        in weird configurations in contact.
        """
    ...
    def set_rom_filters(self):
        """
        Define which type of affordance should be considered to create contact with each
        limb.
    ...
    def load_limbs(
        ...
        see https://github.com/humanoid-path-planner/hpp-rbprm/blob/master \
                /src/sampling/heuristic.cc#L272-L285
        :param analysis: The name of the analysis used,
        see https://github.com/humanoid-path-planner/hpp-rbprm/blob/master \
                /src/sampling/analysis.cc#L318-L335
        :param nb_samples: The number of samples for each limb database. Default is set
        in the Robot python class.
        :param octree_size: The size of each cell of the octree. Default is set in the
        Robot python class.
        """
    ...
    def init_problem(self):
        ...
        if self.path_planner.a_max >= 0:
            self.ps.setParameter(
                "Kinodynamic/accelerationBound", self.path_planner.a_max
            )
    ...
    def init_viewer(self):
        ...
        """
        self.v = Viewer(
            self.ps, viewerClient=self.path_planner.v.client, displayCoM=True
        )

    def compute_configs_from_guide(
        self, use_acc_init=True, use_acc_end=False, set_ref_height=True
    ):
        """
        Compute the wholebody configurations from the reference one and the guide
        init/goal config.
        ...
        :param set_ref_height: if True, set the root Z position of q_init and q_goal to
        be equal to q_ref.
        """
        self.q_init = self.q_ref[::]
        self.q_init[:7] = self.path_planner.ps.configAtParam(self.pid, 0.001)[0:7]
        # do not use 0 but an epsilon in order to avoid the orientation discontinuity
        # that may happen at t=0.
        ...
        self.q_init[configSize : configSize + 3] = q_init_guide[
            index_ecs : index_ecs + 3
        ]
        if use_acc_init:
            self.q_init[configSize + 3 : configSize + 6] = q_init_guide[
                index_ecs + 3 : index_ecs + 6
            ]
        else:
            self.q_init[configSize + 3 : configSize + 6] = [0, 0, 0]
        self.q_goal[configSize : configSize + 3] = q_goal_guide[
            index_ecs : index_ecs + 3
        ]
        if use_acc_end:
            self.q_goal[configSize + 3 : configSize + 6] = q_goal_guide[
                index_ecs + 3 : index_ecs + 6
            ]
        else:
    ...
    def play_guide_path(self):
        ...
        self.cl.problem.selectProblem("guide_planning")
        # this is not the same problem as in the guide_planner. We only added the final
        # path in this problem, thus there is only 1 path in this problem.
