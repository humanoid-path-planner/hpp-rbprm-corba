...
class ContactGenerator(TalosContactGenerator):
    ...
    def load_limbs(self, heuristic):
        ...
        # In this scenario, the arms are not used to create contact, but they may move
        # to avoid collision.
        self.fullbody.addNonContactingLimb(
            self.fullbody.lArmId, self.fullbody.larm, self.fullbody.lhand, 5000
        )
        self.fullbody.runLimbSampleAnalysis(
            self.fullbody.lArmId, "ReferenceConfiguration", True
        )
        self.fullbody.addNonContactingLimb(
            self.fullbody.rArmId, self.fullbody.rarm, self.fullbody.rhand, 5000
        )
        self.fullbody.runLimbSampleAnalysis(
            self.fullbody.rArmId, "ReferenceConfiguration", True
        )
