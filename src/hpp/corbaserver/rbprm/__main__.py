...
parser = argparse.ArgumentParser(
    description="Run a hpp-rbprm scenario. \n"
...
parser.add_argument(
    "-n", "--no_viewer", help="Run rbprm without visualization.", action="store_true"
)
...
scenario_name.replace(
    "/", "."
)  # if the path to the file is given, transform it to a python path
...
with ServerManager("hpp-rbprm-server"):
    ...
    if not args.no_viewer:
        ...
        process_viewer = subprocess.Popen(
            "gepetto-gui",
    ...
    if hasattr(module_scenario, "ContactGenerator"):
        ...
        print(
            "Given script doesn't contain ContactGenerator neither PathPlanner class."
        )
        sys.exit(1)
