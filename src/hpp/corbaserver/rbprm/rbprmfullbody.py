...
class FullBody(Robot):
    ...
    def loadFullBodyModel(self, robotName, rootJointType):
        ...
        self.clientRbprm.rbprm.loadFullBodyRobot(
            robotName,
            rootJointType,
            urdfFilename,
            srdfFilename,
    ...
    # Virtual function to load the fullBody robot model.
    #
    def loadFullBodyModelFromActiveRobot(
        self,
        urdfName,
        rootJointType,
        meshPackageName,
    ...
    def addLimb(self, limbId, name, effectorname, offset, normal, x, y, samples):
        ...
        \\param id: user defined id for the limb. Must be unique.
         The id is used if several contact points are defined for the same limb (ex: the
         knee and the foot)
        \\param name: name of the joint corresponding to the root of the limb.
        \\param effectorName name of the joint to be considered as the effector of the
        limb.
        \\param offset position of the effector in joint coordinates relatively to the
        effector joint.
        \\param unit normal vector of the contact point, expressed in the effector joint
        coordinates.
        ...
        \\param collisionObjects objects to be considered for collisions with the limb.
        TODO remove.
        \\param nbSamples number of samples to generate for the limb
        \\param resolution, resolution of the octree voxels. The samples generated are
        stored in an octree data.
        \\param resolution, resolution of the octree voxels. The samples generated are
        stored in an octree data.
        structure to perform efficient proximity requests. The resulution of the octree,
        in meters, specifies the size.
        of the unit voxel of the octree. The larger they are, the more samples will be
        considered as candidates for.
        contact.
        This can be problematic in terms of performance. The default value is 3 cm.
        """
        self.clientRbprm.rbprm.addLimb(
            limbId, name, effectorname, offset, normal, x, y, samples, "EFORT", 0.03
        )
    ...
    def addLimbDatabase(
        ...
        \\param limbId: user defined id for the limb. Must be unique.
         The id is used if several contact points are defined for the same limb (ex: the
         knee and the foot).
        \\param heuristicName: name of the selected heuristic for configuration
        evaluation
        \\param loadValues: whether values computed, other than the static ones, should
        be loaded in memory.
        \\param disableEffectorCollision: whether collision detection should be disabled
        ...
        if grasp:
            graspv = 1.0
        self.clientRbprm.rbprm.addLimbDatabase(
            databasepath, limbId, heuristicName, boolVal, boolValEff, graspv
        )
        self.limbNames += [limbId]
    ...
    def addLimb(  # noqa
        ...
        contact.
        This can be problematic in terms of performance. The default value is 3 cm.
        \\param contactType whether the contact is punctual ("_3_DOF") or surfacic
        ("_6_DOF").
        \\param disableEffectorCollision: whether collision detection should be disabled
        for end effector bones.
        \\param limbOffset the offset between the limb joint and it's link
        \\param kinematicConstraintsPath : path that point to the .obj file containing
        the kinematic constraints of the.
        limb,
        if not set the default is
        "package://hpp-rbprm-corba/com_inequalities/"+name+"_com_constraints.obj".
        \\param kinematicConstraintsMin : add an additionnal inequalities on the
        kinematicConstraints, of normal (0,0,1).
    ...
    def generateContacts(
        self, configuration, direction, acceleration=[0, 0, 0], robustnessThreshold=0
    ):
        """
        ...
        """
        return self.clientRbprm.rbprm.generateContacts(
            configuration, direction, acceleration, robustnessThreshold
        )

    def generateStateInContact(
        self, configuration, direction, acceleration=[0, 0, 0], robustnessThreshold=0
    ):
        """
        # Generates a balanced contact configuration, considering the
        ...
        """
        return self.clientRbprm.rbprm.generateStateInContact(
            configuration, direction, acceleration, robustnessThreshold
        )

    def generateGroundContact(self, contactLimbs):
        """
        # Generate an autocollision free configuration with limbs in contact with the
        # ground.
    ...
    def createState(self, configuration, contacts, normals=None):
        ...
        for (limbName, normal) in zip(contacts, normals):
            ...
            cl.addNewContact(
                sId, limbName, p, normal, num_max_sample, False, [0, 0, 0, 0]
            )
        return sId
    ...
    def getContactSamplesIds(self, name, configuration, direction):
        return self.clientRbprm.rbprm.getContactSamplesIds(
            name, configuration, direction
        )
    ...
    def getContactSamplesProjected(self, name, configuration, direction, numSamples=10):
        return self.clientRbprm.rbprm.getContactSamplesProjected(
            name, configuration, direction, numSamples
        )
    ...
    # \\param normals  the array describing one normal direction for each limb in
    # contact
    ...
    # criterion required to accept the configuration (0 by default).
    # \\param filterStates If different than 0, the resulting state list will be
    # filtered
    # to remove unnecessary states.
    # \\param testReachability : if true, check each contact transition with our
    # reachability criterion.
    # \\param quasiStatic : if True, use our reachability criterion with the quasiStatic
    # constraint.
    # \\param erasePreviousStates : if True the list of previously computed states is
    # erased.
    def interpolate(
        self,
        stepsize,
        ...
        if filterStates:
            ...
            filt = 0
        return self.clientRbprm.rbprm.interpolate(
            stepsize,
            pathId,
            robustnessTreshold,
            filt,
    ...
    # # Provided a discrete contact sequence has already been computed, computes all the
    # contact positions and normals for a given state, the next one, and the
    # intermediate between them.
    #
    # \\param stateId normalized step for generation along the path (ie the path has a
    # length of 1).
    # \return list of 2 or 3 lists of 6d vectors [pox, poy, posz, nprmalx, normaly,
    # normalz]
    ...
    # \\param isIntermediate whether the intermediate state should be considerred rather
    # than this one.
    ...
    def computeContactPointsAtState(self, stateId, isIntermediate=False):
        rawdata = self.clientRbprm.rbprm.computeContactPointsAtState(
            stateId, isIntermediate
        )
        return [[b[i : i + 6] for i in range(0, len(b), 6)] for b in rawdata][0]

    # # Provided a discrete contact sequence has already been computed, computes
    # all the contact positions and normals for a given state, the next one, and the
    # intermediate between them..
    #
    # \\param stateId normalized step for generation along the path (ie the path has a
    # length of 1)..
    ...
    def computeContactForConfig(self, q, limbName):
        ...
        return [rawdata[i : i + 3] for i in range(0, len(rawdata), 6)], [
            rawdata[i + 3 : i + 6] for i in range(0, len(rawdata), 6)
        ]
    ...
    # \\param pathId Id of the path to compute from
    # \\param robustnessTreshold minimum value of the static equilibrium robustness
    # criterion required to accept the.
    ...
    # \\param quasiStatic : if True, use our reachability criterion with the quasiStatic
    # constraint.
    # \\param erasePreviousStates : if True, the method override the current state list
    # in fullBody,.
    # otherwise it append the results
    def interpolateConfigs(
        self,
        configs,
        robustnessTreshold=0,
        filterStates=False,
        ...
        if filterStates:
            ...
            filt = 0
        return self.clientRbprm.rbprm.interpolateConfigs(
            configs,
            robustnessTreshold,
            filt,
            testReachability,
    ...
    def getLimbsInContact(self, limbNames, state1):
        return [
            limbName for limbName in limbNames if self.isLimbInContact(limbName, state1)
        ]
    ...
    def getLimbsInContactIntermediary(self, limbNames, state1):
        return [
            ...
            if self.isLimbInContactIntermediary(limbName, state1)
        ]
    ...
    def getContactIntermediateCone(self, stateId, friction=0.5):
        H_h = array(
            self.clientRbprm.rbprm.getContactIntermediateCone(stateId, friction)
        )
        # ~ print "H_h", H_h.shape
        # ~ print "norm h", ( H_h[:, -1] != 0).any()
        # now decompose cone
    ...
    # accelerations list contains one less element because it does not have an initial
    # state..
    # a set of 3d key points
    ...
    # \\param positions array of positions
    # \\param partition array of times [0,1.] that describe the beginning or end of each
    # phase.
    ...
    def generateComTraj(self, positions, velocities, accelerations, dt):
        return self.clientRbprm.rbprm.generateComTraj(
    ...
    def generateRootPathStates(self, positions, configState1, configState2):
        return self.clientRbprm.rbprm.generateRootPath(
            positions, configState1[3:7], configState2[3:7]
        )

    # # Given start and goal states
    #  Provided a path has already been computed and interpolated, generate a continuous
    #  path between two indicated states. Will fail if the index of the states do not
    #  exist.
    #
    # \\param index of first state.
    # \\param index of second state.
    # \\param numOptim Number of iterations of the shortcut algorithm to apply between
    # each states.
    #  \return id of the resulting path
    def limbRRT(self, state1, state2, numOptim=10):
        return self.clientRbprm.rbprm.limbRRT(state1, state2, numOptim)

    # # Provided a path has already been computed and interpolated, generate a
    # continuous path.
    # between two indicated states. The states do not need to be consecutive, but
    # increasing in Id..
    ...
    # two states is assumed to be already computed, and registered in the solver under
    ...
    # \\param numOptimizations Number of iterations of the shortcut algorithm to apply
    # between each states.
    #  \return id of the resulting path
    def limbRRTFromRootPath(self, state1, state2, path, numOptim=10):
        return self.clientRbprm.rbprm.limbRRTFromRootPath(
            state1, state2, path, numOptim
        )

    # # Provided a center of mass trajectory has already been computed and interpolated,
    # generate a continuous full.
    # body path between two indicated states. The states do not need to be consecutive,
    # but increasing in Id.
    # Will fail if the index of the states do not exist
    ...
    # increasing in Id.
    # Will fail if the index of the states do not exist
    # The path of the COM of thr robot and limbs not considered by the contact
    # transitions between
    # two states is assumed to be already computed, and registered in the solver under
    # the id specified by the user.
    # It must be valid in the sense of the active PathValidation.
    ...
    def comRRTFromPos(self, state1, comPos1, comPos2, comPos3, numOptim=10):
        return self.clientRbprm.rbprm.comRRTFromPos(
            state1, comPos1, comPos2, comPos3, numOptim
        )
    ...
    # between each states
    # \return id of the root path computed
    def effectorRRTFromPosBetweenState(
        self, state1, state2, comPos1, comPos2, comPos3, numOptim=10
    ):
        return self.clientRbprm.rbprm.effectorRRTFromPosBetweenState(
            state1, state2, comPos1, comPos2, comPos3, numOptim
        )

    def comRRTFromPosBetweenState(
        self, state1, state2, comPos1, comPos2, comPos3, numOptim=10
    ):
        return self.clientRbprm.rbprm.comRRTFromPosBetweenState(
            state1, state2, comPos1, comPos2, comPos3, numOptim
        )
    ...
    def effectorRRT(self, state1, comPos1, comPos2, comPos3, numOptim=10):
        return self.clientRbprm.rbprm.effectorRRT(
            state1, comPos1, comPos2, comPos3, numOptim
        )
    ...
    def effectorRRTFromPath(
        ...
        return self.clientRbprm.rbprm.effectorRRTFromPath(
            state1,
            refPathId,
            path_start,
            path_to,
    ...
    # # Provided a path has already been computed and interpolated, generate a
    # continuous path
    # between two indicated states. The states need to be consecutive with no contact
    ...
    def effectorRRTOnePhase(self, state1, state2, comPos, numOptim=10):
        return self.clientRbprm.rbprm.effectorRRTOnePhase(
            state1, state2, comPos, numOptim
        )

    # # Provided a path has already been computed and interpolated, generate an array of
    # bezier curves,
    ...
    # \\param numOptimizations Number of iterations of the shortcut algorithm to apply
    # between each states
    # \return array of pathIds : first index is the trajectorie, second index is the
    # curve inside this trajectory
    ...
    def generateEffectorBezierArray(self, state1, state2, comPos, numOptim=10):
        return self.clientRbprm.rbprm.generateEffectorBezierArray(
            state1, state2, comPos, numOptim
        )
    ...
    def projectStateToCOM(self, state, targetCom, maxNumSample=0):
        return (
            self.clientRbprm.rbprm.projectStateToCOM(state, targetCom, maxNumSample) > 0
        )
    ...
    # \return whether the projection was successful
    def projectStateToRoot(self, state, root, offset=[0, 0, 0.0]):
    ...
    def runLimbSampleAnalysis(self, limbname, analysis, isstatic):
        ...
        if isstatic:
            isStatic = 1.0
        return self.clientRbprm.rbprm.runLimbSampleAnalysis(
            limbname, analysis, isStatic
        )
    ...
    def computeTargetTransform(self, limbName, configuration, p, n):
        return self.clientRbprm.rbprm.computeTargetTransform(
            limbName, configuration, p, n
        )
    ...
    # # If true, optimize the orientation of all the newly created contact using a
    # postural task
    # \\param use bool
    def usePosturalTaskContactCreation(self, use):
        return self.clientRbprm.rbprm.usePosturalTaskContactCreation(use)

    # # return the time at the given state index (in the path computed during the first
    # phase)
    ...
    # note : it depend on the implementation of interpolate : which state do we add to
    # the list : the first one or the
    ...
    def getDurationForState(self, stateId):
        if stateId == 0:
            ...
            return self.clientRbprm.rbprm.getTimeAtState(
                stateId
            ) - self.clientRbprm.rbprm.getTimeAtState(stateId - 1)

    # # Return the names of all the effector for which a trajectory have been computed
    # for a given path index.
    #  \\param pathId : index of the path, the same index as the wholeBody path stored
    #  in problem-solver
    #  \return the list of all the end-effector (joint names) for which a trajectory
    #  have been defined
    ...
    def getEffectorTrajectoryWaypoints(self, pathId, effectorName):
        return self.clientRbprm.rbprm.getEffectorTrajectoryWaypoints(
            pathId, effectorName
        )
    ...
    #  in problem-solver
    #  \\param effectorName : the name of the desired effector (Joint name)
    #  \return a list of bezier curve (from spline library) followed by the end
    #  effector
    #  Throw an error if there is no trajectory computed for the given id/name
    def getEffectorTrajectory(self, pathId, effectorName):
        curvesWp = self.clientRbprm.rbprm.getEffectorTrajectoryWaypoints(
            pathId, effectorName
        )
        res = []
        for cid in range(len(curvesWp)):
            ...
            if len(wp[0]) == 1:  # length is provided :
                ...
                curve = bezier(waypoints, 0.0, wp[0][0])
            else:
    ...
    # # check the kinematics constraints for the given point, assuming all contact are
    # established
    # \\param point : test if the point is inside the kinematics constraints for the
    ...
    def areKinematicsConstraintsVerifiedForState(self, stateFrom, point):
        return self.clientRbprm.rbprm.areKinematicsConstraintsVerifiedForState(
            stateFrom, point
        )

    def isReachableFromState(
        self, stateFrom, stateTo, computePoint=False, useIntermediateState=True
    ):
        raw = self.clientRbprm.rbprm.isReachableFromState(
            stateFrom, stateTo, useIntermediateState
        )
        if computePoint:
            res = []
            res += [raw[0] > 0.0]
    ...
    def isDynamicallyReachableFromState(
        self,
        stateFrom,
        stateTo,
        ...
        return self.clientRbprm.rbprm.isDynamicallyReachableFromState(
            stateFrom, stateTo, addPathPerPhase, timings, numPointsPerPhases
        )
