...
def compute_state_info(fullBody, states, state_id, phase_dt, mu, computeCones, limbsCOMConstraints, pathId=0):
    ...
    if limbsCOMConstraints is not None:
        ...
        if len(p) > len(COMConstraints):
            COMConstraints.append(get_com_constraint(fullBody, state_id + 1, states[state_id + 1],
                                                     limbsCOMConstraints))
...
def compute_initial_guess(fullBody, t_end_phases, pathId, state_id, dt=0.01):
    ...
    for i in range(0, nbSteps):
        initial_guess[i * 3:3 + i * 3] = fullBody.client.basic.problem.configAtParam(pathId, t_init +
                                                                                     i * dt)[-3:]  # acceleration
...
def gen_trajectory(fullBody,
                   ...
                   pathId=0):
    global lastspeed
    use_window = max(0, min(use_window,
                            (len(states) - 1) - (state_id + 2)))  # can't use preview if last state is reached
    ...
    p, N, init_com, end_com, t_end_phases, cones, COMConstraints = compute_state_info(
        fullBody, states, state_id, phase_dt[:2], mu, computeCones, limbsCOMConstraints, pathId)
    if (not use_velocity):
        initial_guess = []
    if (use_window > 0):
    ...
    for w in range(1, use_window + 1):
        ...
        p1, N1, init_com1, end_com1, t_end_phases1, cones1, COMConstraints1 = compute_state_info(
            fullBody, states, state_id + w, phase_dt[:2], mu, computeCones, limbsCOMConstraints, pathId)
        p += p1
        N += N1
        end_com = end_com1
        cones += cones1
    ...
    print("end_vel = ")
    print(end_vel)
    var_final, params = cone_optimization(p,
                                          N, [init_com + init_vel, end_com + end_vel],
                                          t_end_phases[1:],
                                          dt,
                                          cones,
                                          COMConstraints,
    ...
    if (use_window > 0):
        ...
        if (fullBody.projectStateToCOM(state_id + 1, (var_final['c'][-1]).tolist())):
            ...
            return gen_trajectory(fullBody,
                                  states,
                                  state_id,
                                  computeCones,
                                  mu,
        ...
        if norm(np.array(var_final['c'][-1]) - np.array(__get_com(fullBody, states[state_id + 1]))) > 0.00001:
            ...
            return gen_trajectory(fullBody, states, state_id, computeCones, mu, dt, phase_dt, reduce_ineq, verbose,
                                  limbsCOMConstraints, profile, use_window + 1, use_velocity, pathId)
    lastspeed = np.array([0, 0, 0])

    return var_final, params, timeelapsed


def draw_trajectory(fullBody,
                    states,
                    state_id,
                    computeCones=False,
                    mu=1,
                    ...
                    use_velocity=False,
                    pathId=0):
    var_final, params, elapsed = gen_trajectory(fullBody,
                                                states,
                                                state_id,
                                                computeCones,
                                                mu,
...
def __cVarPerPhase(var, dt, t, final_state, addValue):
    ...
    varPerPhase = [[varVals[(int)(round(t_id / dt))] for t_id in np.arange(t[index], t[index + 1] - _EPS, dt)]
                   for index, _ in enumerate(t[:-1])]
    # print("varperPhase =")
    # print(varPerPhase)
    varPerPhase[2].append(varVals[-1])
    if var != "ddc":
        assert len(varVals) == len(varPerPhase[0]) + len(varPerPhase[1]) + len(varPerPhase[2])
    ...
    if var == "ddc":  # acceleration: remove first
        ...
        assert len(final_state[var]) == len(varPerPhase[0]) + len(varPerPhase[1]) + len(
            varPerPhase[2]), "incorrect num of ddc"
    return varPerPhase


def __optim__threading_ok(fullBody,
                          states,
                          state_id,
                          computeCones=False,
                          mu=1,
    ...
    if (draw):
        res = draw_trajectory(fullBody, states, state_id, computeCones, mu, dt, phase_dt, reduce_ineq, verbose,
                              limbsCOMConstraints, use_window, use_velocity, pathId)
    else:
        res = gen_trajectory(fullBody, states, state_id, computeCones, mu, dt, phase_dt, reduce_ineq, verbose,
                             limbsCOMConstraints, profile, use_window, use_velocity, pathId)
    alpha = res[1]['alpha']
    ...
    comTrajIds = [
        fullBody.generateComTraj(comPosPerPhase[i], comVelPerPhase[i], comAccPerPhase[i], dt) for i in range(0, 3)
    ]
    return comTrajIds, res[2], final_state  # res[2] is timeelapsed


def solve_com_RRT(fullBody,
                  states,
                  state_id,
                  computeCones=False,
                  mu=1,
                  ...
                  use_velocity=False,
                  pathId=0):
    comPosPerPhase, timeElapsed, final_state = __optim__threading_ok(fullBody, states, state_id, computeCones, mu, dt,
                                                                     phase_dt, reduce_ineq, num_optims, draw, verbose,
                                                                     limbsCOMConstraints, profile, use_window,
                                                                     use_velocity, pathId)
    print("done. generating state trajectory ", state_id)
    paths_ids = [
        int(el)
        for el in fullBody.comRRTFromPos(state_id, comPosPerPhase[0], comPosPerPhase[1], comPosPerPhase[2], num_optims)
    ]
...
def solve_effector_RRT(fullBody,
                       states,
                       state_id,
                       computeCones=False,
                       mu=1,
    ...
    if (len(trackedEffectors) == 0):
        paths_ids = [
            int(el) for el in fullBody.effectorRRT(state_id, comPosPerPhase[0], comPosPerPhase[1], comPosPerPhase[2],
                                                   num_optims)
        ]
    else:
        ...
        paths_ids = [
            int(el)
            for el in fullBody.effectorRRTFromPath(state_id, refPathId, path_start, path_to, comPosPerPhase[0],
                                                   comPosPerPhase[1], comPosPerPhase[2], num_optims, effectorstracked)
        ]
    print("done. computing final trajectory to display ", state_id, "path ids ", paths_ids[-1], " ,", paths_ids[:-1])
    return paths_ids[-1], paths_ids[:-1], timeElapsed, final_state
