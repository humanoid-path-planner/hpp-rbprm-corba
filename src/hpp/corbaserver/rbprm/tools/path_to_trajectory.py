...
def __gen_frame_positions(com_waypoints, dt, dt_framerate=__24fps):
    ...
    for i in range(len(com_waypoints) - 1):
        ids_val += [
            (i, val - dt * i)
            for val in dt_finals
            if (val < (i + 1) * dt) and (val > i * dt)
        ]
    return (
        [com_waypoints[0]]
        + [
            __linear_interpolation(com_waypoints[i], com_waypoints[i + 1], dt, val)
            for (i, val) in ids_val
        ]
        + [com_waypoints[-1]]
    )
...
def __find_q_t(robot, path_player, path_id, t):
    ...
    a = 0.0
    b = length
    # ~ print([pp.client.problem.configAtParam (path_id, i)[-1] for i in [length / 5. *j
    # for j in range(6)]])
    while True:
...
def linear_interpolate_path(
    robot, path_player, path_id, total_time, dt_framerate=__24fps
):
    pp = path_player
    length = pp.client.problem.pathLength(path_id)
...
def follow_trajectory_path(
    ...
    # ~ print("length ", length, "total tome ", total_time, "frame rate ", dt_framerate,
    # "num_frames_required ", num_frames_required, "dt ", dt)
...
def gen_trajectory_to_play(
    robot, path_player, path_ids, total_time_per_paths, dt_framerate=__24fps
):
    ...
    for i, path_id in enumerate(path_ids):
        config_size_path = len(
            path_player.client.problem.configAtParam(int(path_id), 0)
        )
        if config_size_path > config_size:
            # ~ if(i == 1 ):
            res += follow_trajectory_path(
                robot, path_player, int(path_id), total_time_per_paths[i], dt_framerate
            )
        else:
            res += linear_interpolate_path(
                robot, path_player, int(path_id), total_time_per_paths[i], dt_framerate
            )
        activeid += 1
    return res
