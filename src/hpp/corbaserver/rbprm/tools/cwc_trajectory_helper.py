...
def genPandNperFrame(fullBody, stateid, limbsCOMConstraints, pp, path_ids, times, dt_framerate=1. / 24.):
    ...
    freeEffectors = [[
        limbsCOMConstraints[limb]['effector'] for limb in limbsCOMConstraints.keys()
...
def __getTimes(fullBody, configs, i, time_scale):
    ...
    total_time_support_path = float((int)(ceil(min(total_time_flying_path / 2., 0.2) * 10.))) / 10.
...
def __getTimes(fullBody, configs, i, time_scale, use_window=0):
    ...
    """
    times[1] = t - 2 * times[0]
    times[1] = float((int)(floor(times[1] * 100.))) / 100.
    print("times : ", times)
...
def step(fullBody,
    ...
    if (True):
        ...
        use_window = max(0, min(use_window,
                                (len(configs) - 1) - (i + 2)))  # can't use preview if last state is reached
        w = 0
        ...
        if (distance > 0.0001):
            ...
            if (useCOMConstraints):
                ...
                comC = None
            if (optim_effectors):
                pid, trajectory, timeelapsed, final_state = solve_effector_RRT(fullBody,
                                                                               configs,
                                                                               i,
                                                                               True,
                                                                               friction,
                ...
                pid, trajectory, timeelapsed, final_state = solve_com_RRT(fullBody,
                                                                          configs,
                                                                          i,
                                                                          True,
                                                                          friction,
            ...
            # ~ new_contacts = gencontactsPerFrame(fullBody, i, limbsCOMConstraints, pp, trajectory, times,
            # frame_rate_andrea)
            Ps, Ns, freeEffectorsPerPhase = genPandNperFrame(fullBody, i, limbsCOMConstraints, pp, trajectory,
                                                             time_per_path, frame_rate_andrea)
            NPeffs = genPEffperFrame(fullBody, freeEffectorsPerPhase, new_traj_andrea, pp, time_per_path,
                                     frame_rate_andrea)
            ...
            # assert(len(trajec_mil) == len(pos) and len(normals) == len(pos) and len(normals) == len(coms) and
            # len(coms) == len(pEffs))
    ...
    # ~ try:
    # ~ return step(fullBody, configs, i, optim, pp, limbsCOMConstraints,  friction, optim_effectors, time_scale,
    # useCOMConstraints, 1, verbose, draw, trackedEffectors = trackedEffectors)
    # ~ except:
    # ~ if ((len(configs) - 1) - (i + 3) > 0):
    # ~ print "could not project com, trying to increase velocity more "
    # ~ step(fullBody, configs, i, optim, pp, limbsCOMConstraints,  friction, optim_effectors, time_scale,
    # useCOMConstraints, 2, verbose, draw,  trackedEffectors = trackedEffectors)
    # ~ else:
    ...
    try:
        ...
        if (distance > 0.0001):
            ...
            if (useCOMConstraints):
                ...
                comC = None
            if (optim_effectors):
                pid, trajectory, timeelapsed, final_state = solve_effector_RRT(fullBody, configs, i, True, friction,
                                                                               dt, times, False, optim, False, False,
                                                                               comC, True)
            else:
                pid, trajectory, timeelapsed, final_state = solve_com_RRT(fullBody, configs, i, True, friction, dt,
                                                                          times, False, optim, False, False, comC,
                                                                          True)
...
def write_stats(filename):
    ...
    f.write("time_cwc " + str(sd["time_cwc"]["min"]) + " " + str(sd["time_cwc"]["avg"]) + " " +
            str(sd["time_cwc"]["max"]) + " " + str(sd["time_cwc"]["totaltime"]) + " " +
            str(sd["time_cwc"]["numiter"]) + " " + "\n")
    f.close()
    return sd


def profile(fullBody,
            configs,
            i_start,
            i_end,
            limbsCOMConstraints,
    ...
    for i in range(i_start, i_end):
        step_profile(fullBody, configs, i, 0, limbsCOMConstraints, friction, optim_effectors, time_scale,
                     useCOMConstraints)
