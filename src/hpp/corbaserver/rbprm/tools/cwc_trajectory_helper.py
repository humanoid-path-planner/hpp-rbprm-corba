...
from hpp.corbaserver.rbprm.tools.cwc_trajectory import solve_com_RRT, solve_effector_RRT
...
def displayComPath(pp, pathId, color=[0.0, 0.75, 0.15, 0.9]):
    pathPos = []
...
def genPandNperFrame(
    fullBody, stateid, limbsCOMConstraints, pp, path_ids, times, dt_framerate=1.0 / 24.0
):
    p, N = fullBody.computeContactPointsPerLimb(
        stateid, limbsCOMConstraints.keys(), limbsCOMConstraints
    )
    freeEffectors = [
        [
            limbsCOMConstraints[limb]["effector"]
            for limb in limbsCOMConstraints.keys()
            if limbsCOMConstraints[limb]["effector"] not in p[i]
        ]
...
def genComPerFrame(final_state, dt, dt_framerate=1.0 / 1000.0):
    num_frames_per_dt = int(round(dt / dt_framerate))
    inc = 1.0 / ((float)(num_frames_per_dt))
...
def __getTimes(fullBody, configs, i, time_scale):
    trunk_distance = np.linalg.norm(np.array(configs[i + 1][0:3]) -
    np.array(configs[i][0:3]))
    ...
    total_time_support_path = float((int)(ceil(min(total_time_flying_path / 2., 0.2) *
    10.))) / 10.
...
def __getTimes(fullBody, configs, i, time_scale, use_window=0):
    ...
    trunk_distance = np.linalg.norm(
        np.array(configs[i + 1][0:3]) - np.array(configs[i][0:3])
    )
    distance = max(fullBody.getEffectorDistance(i, i + 1), trunk_distance)
    ...
    if t <= dt * 6.:
        ...
        use_window = 2
    """
    # FIXME : hardcoded value depend on interpolation step choosen (not available here)
    times = [
        dt * 5.0,
        0,
    ]
    ...
    """
    times[1] = t - 2 * times[0]
    times[1] = float((int)(floor(times[1] * 100.0))) / 100.0
    print("times : ", times)
...
def step(
    ...
    if True:
        times = []
        dt = 1000
        times, dt, distance, use_window = __getTimes(
            fullBody, configs, i, time_scale, use_window
        )
        print("Use window = ", use_window)
        if distance == 0:
            use_window = use_window + 1
        use_window = max(
            0, min(use_window, (len(configs) - 1) - (i + 2))
        )  # can't use preview if last state is reached
        w = 0
        while w < (use_window + 1):
            times2, dt2, dist2, use_window = __getTimes(
                fullBody, configs, i + w, time_scale, use_window
            )
            print("Use window = ", use_window)
            times += times2
            dt = min(dt, dt2)
        ...
        if distance > 0.0001:
            ...
            if useCOMConstraints:
                ...
                comC = None
            if optim_effectors:
                pid, trajectory, timeelapsed, final_state = solve_effector_RRT(
                    fullBody,
                    configs,
                    i,
                    True,
                ...
                pid, trajectory, timeelapsed, final_state = solve_com_RRT(
                    fullBody,
                    configs,
                    i,
                    True,
            ...
            global trajec_mil
            frame_rate = 0.01
            frame_rate_andrea = 1.0 / 100.0
            # 			frame_rate_andrea = 1./1000.
            # ~ if(len(trajec) > 0):
            # ~ frame_rate = 1./25.
            # ~ frame_rate_andrea = 1./1001.
            print("first traj :")
            new_traj = gen_trajectory_to_play(
                fullBody, pp, trajectory, time_per_path, frame_rate
            )
            print("traj Andrea : ")
            new_traj_andrea = gen_trajectory_to_play(
                fullBody, pp, trajectory, time_per_path, frame_rate_andrea
            )
            # ~ new_contacts = gencontactsPerFrame(fullBody, i, limbsCOMConstraints, pp,
            # trajectory, times, frame_rate_andrea)
            Ps, Ns, freeEffectorsPerPhase = genPandNperFrame(
                fullBody,
                i,
                limbsCOMConstraints,
                pp,
            ...
            global coms
            coms += com
            # print(len(trajec_mil), " ",  len(pos), " ", len(normals), " ", len(coms),
            # " ", len(pEffs))
            # assert(len(trajec_mil) == len(pos) and len(normals) == len(pos) and
            # len(normals) == len(coms) and
            # len(coms) == len(pEffs))
    ...
    # ~ try:
    # ~ return step(fullBody, configs, i, optim, pp, limbsCOMConstraints,  friction,
    # optim_effectors, time_scale, useCOMConstraints, 1, verbose, draw, trackedEffectors
    # = trackedEffectors)
    # ~ except:
    # ~ if ((len(configs) - 1) - (i + 3) > 0):
    # ~ print "could not project com, trying to increase velocity more "
    # ~ step(fullBody, configs, i, optim, pp, limbsCOMConstraints,  friction,
    # optim_effectors, time_scale, useCOMConstraints, 2, verbose, draw,
    # trackedEffectors = trackedEffectors)
    # ~ else:
    ...
    # ~ try:
    # ~ return step(fullBody, configs, i, optim, pp, limbsCOMConstraints,  friction,
    # optim_effectors, time_scale, useCOMConstraints, 1, verbose, draw,
    # trackedEffectors = trackedEffectors)
    # ~ except:
    # ~ print "faile twice"
    # ~ if ((len(configs) - 1) - (i + 3) > 0):
...
def step_profile(
    ...
    try:
        ...
        while dist % 4 != 0:
            dist += 1
        total_time_flying_path = max(float(dist) / 10.0, 0.3)
        total_time_support_path = (
            float((int)(ceil(min(total_time_flying_path / 2.0, 0.2) * 10.0))) / 10.0
        )
        times = [total_time_support_path, total_time_flying_path]
        if total_time_flying_path >= 1.0:
            dt = 0.1
        elif total_time_flying_path <= 0.3:
            dt = 0.05
        else:
...
def __isDiff(P0, P1):
    return len(set(P0.keys()) - set(P1.keys())) != 0 or len(
        set(P1.keys()) - set(P0.keys())
    )
...
def write_stats(filename):
    ...
    f.write(
        ...
        + str(sd["time_cwc"]["max"])
        + " "
        + str(sd["time_cwc"]["totaltime"])
        + " "
        + str(sd["time_cwc"]["numiter"])
        + " "
        + "\n"
    )
    f.close()
...
def profile(
    ...
    stat_data["time_cwc"]["avg"] = stat_data["time_cwc"]["totaltime"] / float(
        stat_data["time_cwc"]["numiter"]
    )
