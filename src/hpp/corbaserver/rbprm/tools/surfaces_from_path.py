...
ROBOT_NAME = "talos"
# if a contact surface is greater than this value, the intersection is used instead of
# the whole surface.
MAX_SURFACE = 1.0
LF = 0
RF = 1
...
def getConfigsFromPath(ps, pathId=0, discretisationStepSize=1.0):
    configs = []
...
def getAllSurfaces(afftool):
    ...
    return [(getSurfaceExtremumPoints(el), normal(el[0])) for el in supports]
...
def getContactsNames(rbprmBuilder, i, q, use_all_limbs=False):
    ...
    if i % 2 == LF:  # left leg
        step_contacts = rbprmBuilder.clientRbprm.rbprm.getCollidingObstacleAtConfig(
            q, rbprmBuilder.lLegId
        )
    elif i % 2 == RF:  # right leg
        step_contacts = rbprmBuilder.clientRbprm.rbprm.getCollidingObstacleAtConfig(
            q, rbprmBuilder.rLegId
        )
    return step_contacts
...
def getSurfacesFromGuideContinuous(
    ...
    all_names = afftool.getAffRefObstacles("Support")  # id in names and surfaces match
    # map surface names to surface points
    surfaces_dict = dict(zip(all_names, all_surfaces))
    # list of list of surfaces : for each phase contain a list of surfaces. One phase is
    # defined by moving of 'step' along the path.
    seqs = []
    configs = []
    ...
    while not end:  # for all the path
        # print "Looking for surfaces for phase "+str(len(seqs))+" for t in
        # ["+str(t+discretizationStep)+" ; "+str(current_phase_end)+" ] ".
        phase_contacts_names = []
        rot_valid = True
        while t < current_phase_end and rot_valid:
            # get the names of all the surfaces that the rom collide while moving from
            # current_phase_end-step to current_phase_end.
            t += discretizationStep
            ...
            for contact_name in step_contacts:
                if contact_name not in phase_contacts_names:
        ...
        if useIntersection:
            ...
            # First add intersection for the surfaces in contact at the last
            # discretization step.
            ...
            # then add possible small surfaces encountered during the discretization of
            # the path.
        ...
        # print "There was "+str(len(phase_contacts_names))+" surfaces in contact during
        # this phase.".
        # why is this step required ? without out the lp can fail
        phase_surfaces = sorted(phase_surfaces)
        # convert from list to array, we cannot do this before because sorted() require
        # list.
        ...
        configs.append(q)
        i += 1
        if t >= (pathLength - discretizationStep / 2.0):
            end = True
        # because we want the first iteration of the next phase to test the same t as
        # the last iter of this phase.
        t -= discretizationStep
...
def getSurfacesFromGuide(
    ...
    for i, q in enumerate(configs):
        seq = []
        intersections = getContactsIntersections(
            rbprmBuilder, i, q
        )  # get intersections at config
        phase_contacts_names = getContactsNames(
            rbprmBuilder, i, q
        )  # get the list of names of the surface in contact at config
        for j, intersection in enumerate(intersections):
            if (
                useIntersection and area(intersection) > MAX_SURFACE
            ):  # append the intersection
                seq.append(intersection)
            else:
                if len(intersections) == len(phase_contacts_names):
                    # in case getCollidingObstacleAtConfig does not work (because of the
                    # rom?).
                    # append the whole surface
                    seq.append(surfaces_dict[phase_contacts_names[j]][0])
                else:
