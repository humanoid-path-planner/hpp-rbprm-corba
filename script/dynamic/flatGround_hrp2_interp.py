...
rLegy = 0.05
fullBody.addLimb(
    rLegId, rLeg, "", rLegOffset, rLegNormal, rLegx, rLegy, 50000, "forward", 0.1
)
...
lLegy = 0.05
fullBody.addLimb(
    lLegId, lLeg, "", lLegOffset, rLegNormal, lLegx, lLegy, 50000, "forward", 0.1
)
...
q_init[0:7] = tp.ps.configAtParam(pId, 0.0001)[
    0:7
...
dir_goal = tp.ps.configAtParam(pId, tp.ps.pathLength(pId) - 0.0001)[
...
configs = fullBody.interpolate(
    0.01, pathId=pId, robustnessTreshold=1, filterStates=True
)
print("number of configs :", len(configs))


player = fullBodyPlayerHrp2.Player(
    fullBody,
    pp,
    tp,
    configs,
