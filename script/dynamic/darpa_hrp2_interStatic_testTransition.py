...
fullBody.addLimb(
    ...
    kinematicConstraintsPath="package://hpp-rbprm-corba/com_inequalities/empty_com_constraints.obj",
)
...
f = open(
    "/local/fernbac/bench_iros18/kin_constraint_tog/without_kin_constraints.log", "a"
)
...
if num_transition > 0:
    f.write(
        "valid_transition_percent "
        + str(float(valid_transition / num_transition) * 100.0)
        + " %\n"
    )
...
if length_traj > 0:
    f.write(
        "valid_traj_percent       "
        + str(float(valid_length / length_traj) * 100.0)
        + " %\n"
    )
