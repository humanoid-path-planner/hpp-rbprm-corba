...
statesPerStep = (
    5  # number of double support configs from the planning per call to muscod
)
stepSize = (
    statesPerStep * 2 - 1
    ...
    for i in range(0, numStep):
        if i == (numStep - 1):
            ...
            currentSteps = (stepSize - 3) * (
                numStep - 1
            )  # id of the next phase to compute
            ...
            for k in range(cs.size() - currentSteps):
                sequences[i].contact_phases[k] = cs.contact_phases[
                    currentSteps + k
                ]  # last phases of sequence i is the second phase of sequence i+1, not the first
        else:
        ...
        if i > 0:
            sequences[i].contact_phases[0].init_state = (
                outputs[i - 1]
                .contact_phases[stepSize - 3]
                .state_trajectory[P3_Num_step]
            )
            sequences[i].contact_phases[0].reference_configurations[0] = (
                outputs[i - 1].contact_phases[stepSize - 3].reference_configurations[0]
            )
        ...
        outputs[i].loadFromXML(
            CONTACT_SEQUENCE_WHOLEBODY_FILE, CONTACT_SEQUENCE_XML_TAG
        )
        print(
            "cut contact sequence, call " + str(i) + " size = " + str(outputs[i].size())
        )

    ## merge sequences outputs together :
    init_time_at_step = 0.0
    ...
    for id_steps in range(0, numStep):
        if id_steps > 0:
            ...
            for j in range(0, P3_Num_step):  # first half, take mid_phase0
                ...
                mid_phase1.time_trajectory[j] = (
                    mid_phase0.time_trajectory[j] + init_time_at_step
                )
            init_time_at_step += mid_phase0.time_trajectory[
                P3_Num_step
            ]  # need to offset all the times of the next phases by this value
            for j in range(
                P3_Num_step, len(mid_phase1.state_trajectory)
            ):  # second half, take and expand mid_phase1
                mid_phase1.state_trajectory[j] = old_state_traj[(j - P3_Num_step) * 2]
                mid_phase1.time_trajectory[j] = (
                    old_time_traj[(j - P3_Num_step) * 2] + init_time_at_step
                )
        ...
        for id_phase in range(1, stepSize - 3):
            ...
            for i in range(0, len(phase.time_trajectory)):
                phase.time_trajectory[
                    i
                ] += init_time_at_step  # offset all the times by the last value of the last step
