    ...
    def __init__(self, *args):
        ...
        if len(args) == 0:  # By default, if no argument is given
            ...
            if type(args[0]) == Quaternion:  # From a Quaternion
                ...
                np.array(args[0]).size == 1
            ):  # From one sized element, this element will be the scalar part, the vector part will be set at (0,0,0)
                ...
                np.array(args[0]).size == 3 and max(np.array(args[0]).shape) == 3
            ):  # From an array, matrix, tuple or list of 3 elements interpreted as a rotation vector
            ...
            ):  # From a to 2 dimension array convertible array, matrix, tuple or list with at least (3*3) elements interpreted  as a rotation matrix
        ...
        if error:
            raise TypeError(
                "Impossible to instanciate the Quaternion object with the given arguments"
            )
