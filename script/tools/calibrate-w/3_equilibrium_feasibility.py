...
rArmy = 0.024
fullBody.addLimb(
    rarmId, rarm, rHand, rArmOffset, rArmNormal, rArmx, rArmy, 10000, "EFORT", 0.05
)
...
lArmy = 0.024
fullBody.addLimb(
    larmId, larm, lHand, lArmOffset, lArmNormal, lArmx, lArmy, 10000, "EFORT", 0.05
)
...
f.write(
    "size 1 \n \t true false negative "
    + str(float(trueFalseNegative) / float(len(falseNeg)))
    + "\n"
)
f.write(
    "size 1 \n \t false false negative "
    + str(float(falseFalseNegative) / float(len(falseNeg)))
    + "\n"
)
f.close()
