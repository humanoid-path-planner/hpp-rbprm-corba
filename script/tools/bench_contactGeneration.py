    ...
    if tryWarmStart:
        if not success:
            ...
            if Path(CONTACT_SEQUENCE_WHOLEBODY_FILE).is_file():
                os.remove(CONTACT_SEQUENCE_WHOLEBODY_FILE)
            filename_xml = OUTPUT_DIR + "/" + OUTPUT_SEQUENCE_FILE
            cs = generateContactSequenceWithInitGuess(
                ps, fullBody, configs, beginState, endState
            )
            ...
            muscodWarmStartConverged = (
                successMuscod and Path(CONTACT_SEQUENCE_WHOLEBODY_FILE).is_file()
            )
        else:
