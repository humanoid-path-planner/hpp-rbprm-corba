...
def check_acceleration_bounds(ddc_qp, t_discretized):
    ...
    for t in t_discretized:
        ...
        for i in range(3):
            if abs(a[i]) > max_acc:
                print(
                    "Acceleration sup to bound, at time : "
                    + str(t)
                    + "    a = "
                    + str(a)
                )
...
def check_stability(s, c, dc, ddc, t):
    ...
    success = s.fullBody.isConfigBalanced(
        q, s.getLimbsInContact(), CoM=c(t).transpose().tolist()[0]
    )
    if not success:
...
def check_muscod_traj(fullBody, cs, s0_, s1_):
    ...
    for k in range(3):
        ...
        for i in range(len(c_array)):
            ...
            if stab_valid_discretized and not states_copy[k].fullBody.isConfigBalanced(
                q, states_copy[k].getLimbsInContact(), CoM=c_array[i].tolist()[0]
            ):
        ...
        for i in range(int(len(c_array) / 100)):
            ...
            if stab_valid and not states_copy[k].fullBody.isConfigBalanced(
                q, states_copy[k].getLimbsInContact(), CoM=c_array[i * 100].tolist()[0]
            ):
                stab_valid = False
...
def check_contact_plan(ps, r, pp, fullBody, idBegin, idEnd):
    ...
    for id_state in range(idBegin, idEnd - 1):
        print(
            "#### check for transition between state "
            + str(id_state)
            + " and "
            + str(id_state + 1)
        )
