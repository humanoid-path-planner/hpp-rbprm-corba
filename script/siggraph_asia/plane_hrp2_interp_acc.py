...
rLegy = 0.05
fullBody.addLimb(
    rLegId, rLeg, "", rLegOffset, rLegNormal, rLegx, rLegy, 10000, "manipulability", 0.1
)
...
lLegy = 0.05
fullBody.addLimb(
    lLegId, lLeg, "", lLegOffset, rLegNormal, lLegx, lLegy, 10000, "manipulability", 0.1
)
...
def test(stateid=1, path=False, use_rand=False, just_one_curve=False):
    ...
    c_bounds_1 = get_com_constraint(
        fullBody, stateid, configs[stateid], limbsCOMConstraints, interm=False
    )
    c_bounds_mid = get_com_constraint(
        fullBody, stateid, configs[stateid], limbsCOMConstraints, interm=True
    )
    c_bounds_2 = get_com_constraint(
        fullBody, stateid, configs[stateid + 1], limbsCOMConstraints, interm=False
    )
    success, c_mid_1, c_mid_2 = solve_quasi_static(
        data, c_bounds=[c_bounds_1, c_bounds_2, c_bounds_mid], use_rand=use_rand
    )
    ...
    if path and success:
        ...
        if just_one_curve:
            bezier_0 = __Bezier(
                ...
                init_acc=[0.0, -20.0, 0.0],
                end_acc=[0.0, 0.0, 0.0],
                init_vel=[1.0, 0.0, 0.0],
                end_vel=[1.0, 0.0, 0.0],
            )

            p0 = fullBody.generateCurveTrajParts(bezier_0, [0.0, 0.1, 0.9, 1.0])
            print("p0", p0)
            # ~ pp.displayPath(p0+1)
            # ~ pp.displayPath(p0+2)
            pp.displayPath(p0)
            paths_ids = [
                int(el)
                for el in fullBody.comRRTFromPos(stateid, p0 + 1, p0 + 2, p0 + 3)
            ]
            # ~ paths_ids = [int(el) for el in fullBody.effectorRRT(stateid,p0+1,p0+2,p0+3)]
        else:
            bezier_0 = __Bezier(
                [com_1, c_mid_1[0].tolist()],
            ...
            pp.displayPath(p0 + 2)
            paths_ids = [
                int(el) for el in fullBody.comRRTFromPos(stateid, p0, p0 + 1, p0 + 2)
            ]
        # ~ paths_ids = []
        global allpaths
        ...
        for i in range(0, num_paths):
            ...
            paths_ids += [
                ...
                for el in fullBody.comRRTFromPos(
                    stateid + i, p0 + 3 * i, p0 + 3 * i + 1, p0 + 3 * i + 2
                )
            ]
            # ~ paths_ids = []
            global allpaths
...
a = gen_trajectory_to_play(
    fullBody, pp, allpaths, flatten([[0.1, 0.9, 0.1] for _ in range(len(allpaths) / 3)])
)
